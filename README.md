...
npx nx serve docs    # http://localhost:4200 — per-component docs site (live examples + API)
```
...
│       └── overlay/             # dialog
├── apps/
│   └── docs/                  # per-component documentation site — live examples + API tables, plus a Playwright e2e/visual-regression target
└── tools/
...
and call `provideDynamoNG({ theme: 'aura' })` in `app.config.ts`. See `apps/docs/src/styles.css` for a
...
- **Coverage**: enforced via `nx.json` `targetDefaults` (applies to every project automatically) —
  ...
  `coverageExclude` so a component's coverage isn't diluted by its dependencies' code. `apps/docs` skips
  coverage entirely (`coverage: false`) — it's an integration smoke test, not the primary testable unit.
...
- **Visual regression testing is scaffolded, not yet a working gate**: `apps/docs` has a Playwright `e2e`
  ...
  Windows dev machine, since cross-platform font rendering would produce false-positive diffs. Coverage is
  being migrated page-by-page as each component doc page moves to the new examples layout.
...
type="search">`, wired via `[ngModel]`/`(ngModelChange)` since `DynamoInputText` has no direct value
  ...
  the docs "Pagination" page still separately demos composing a bare `dg-table` (with its own
