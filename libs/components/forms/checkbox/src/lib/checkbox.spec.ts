...
describe('DynamoCheckbox', () => {
  ...
  describe('default behavior', () => {
    ...
    it('defaults to not disabled', () => {
      ...
      expect((within(container).getByRole('checkbox') as HTMLInputElement).disabled).toBe(false);
    });
  });

  describe('input properties', () => {
    it('reflects the checked input onto the native input element', () => {
      const { container } = renderDynamoComponent(DynamoCheckbox, { inputs: { checked: true } });
    ...
    it('reflects the indeterminate input as a DOM property (not an attribute)', () => {
      const { container } = renderDynamoComponent(DynamoCheckbox, { inputs: { indeterminate: true } });

      expect((within(container).getByRole('checkbox') as HTMLInputElement).indeterminate).toBe(true);
    });

    it('reflects the disabled input onto the native input element', () => {
      const { container } = renderDynamoComponent(DynamoCheckbox, { inputs: { disabled: true } });

      expect((within(container).getByRole('checkbox') as HTMLInputElement).disabled).toBe(true);
    });

    it('accepts every documented size without throwing', () => {
      const { componentInstance, setInputs } = renderDynamoComponent(DynamoCheckbox);
  ...
  describe('output events', () => {
    it('updates a two-way-bound value when the checkbox is toggled', async () => {
      const { container, componentInstance } = renderDynamoComponent(CheckboxTwoWayHostComponent);
  ...
  describe('user interactions', () => {
    it('toggles when clicked directly', async () => {
      const { container, componentInstance } = renderDynamoComponent(DynamoCheckbox);
    ...
    it('supports interaction through the DynamoCheckboxHarness', async () => {
      ...
      const harness = await TestbedHarnessEnvironment.harnessForFixture(fixture, DynamoCheckboxHarness);
  ...
  describe('conditional rendering', () => {
    ...
    it('renders the indeterminate dash instead of the checkmark when both are true', () => {
      const { container } = renderDynamoComponent(DynamoCheckbox, { inputs: { checked: true, indeterminate: true } });
  ...
  describe('template behavior', () => {
    it('applies different box classes for the checked vs. unchecked state', () => {
      const { container, setInputs } = renderDynamoComponent(DynamoCheckbox, { inputs: { checked: false } });
  ...
  describe('accessibility', () => {
    ...
    it('sets aria-checked="mixed" while indeterminate', () => {
      ...
      expect(within(container).getByRole('checkbox').getAttribute('aria-checked')).toBe('mixed');
    });

    it('does not set aria-checked when not indeterminate (native checked state is sufficient)', () => {
      const { container } = renderDynamoComponent(DynamoCheckbox, { inputs: { checked: true } });

      expect(within(container).getByRole('checkbox').getAttribute('aria-checked')).toBeNull();
    });
  });

  describe('state changes', () => {
    it('cannot be toggled by click when disabled', async () => {
      const { container, componentInstance } = renderDynamoComponent(DynamoCheckbox, { inputs: { disabled: true } });
    ...
    it('becomes toggleable again once disabled is set back to false', async () => {
      const { container, componentInstance, setInputs } = renderDynamoComponent(DynamoCheckbox, {
  ...
  describe('edge cases', () => {
    ...
    it('generates a distinct id for each checkbox instance sharing the same injector', () => {
      @Component({
        ...
        standalone: true,
        imports: [DynamoCheckbox],
        template: `<dg-checkbox>First</dg-checkbox><dg-checkbox>Second</dg-checkbox>`,
      })
