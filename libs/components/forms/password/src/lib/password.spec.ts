...
describe('DynamoPassword', () => {
  ...
  describe('input properties', () => {
    it('reflects the placeholder input', () => {
      ...
      expect(
        within(container).getByPlaceholderText('Enter password'),
      ).toBeTruthy();
    });
    ...
    it('only sets aria-invalid when invalid is true', () => {
      const { container, setInputs } = renderDynamoComponent(DynamoPassword, {
        inputs: { ariaLabel: 'Password' },
      });
      expect(
  ...
  describe('user interactions', () => {
    ...
    it('supports interaction through the DynamoPasswordHarness', async () => {
      const { fixture } = renderDynamoComponent(DynamoPassword, {
        inputs: { ariaLabel: 'Password' },
      });
      const harness = await TestbedHarnessEnvironment.harnessForFixture(
        fixture,
        DynamoPasswordHarness,
      );
  ...
  describe('strength meter', () => {
    ...
    it('honors custom weak/medium/strong labels', () => {
      const { fixture, container } = renderDynamoComponent(DynamoPassword, {
        inputs: {
          ariaLabel: 'Password',
      ...
      fireEvent.input(input, { target: { value: 'abc' } });
      fixture.detectChanges();
      expect(container.textContent).toContain('Faible');

      fireEvent.input(input, { target: { value: 'Abcdefghij12!@' } });
      fixture.detectChanges();
      expect(container.textContent).toContain('Fort');
    });
  });

  describe('showToggle', () => {
    it('renders the show/hide button by default', () => {
      const { container } = renderDynamoComponent(DynamoPassword, {
        inputs: { ariaLabel: 'Password' },
      });
    ...
    it('omits the show/hide button when showToggle is false', () => {
      const { container } = renderDynamoComponent(DynamoPassword, {
        inputs: { ariaLabel: 'Password', showToggle: false },
      });
