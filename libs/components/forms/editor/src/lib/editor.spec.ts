...
describe('DynamoEditor', () => {
  ...
  describe('creation', () => {
    it('renders a contenteditable textbox and all six toolbar buttons', () => {
      ...
      expect(
        within(container).getByRole('button', { name: 'Bulleted list' }),
      ).toBeTruthy();
      expect(
        within(container).getByRole('button', { name: 'Numbered list' }),
      ).toBeTruthy();
      expect(
        within(container).getByRole('button', { name: 'Insert link' }),
      ).toBeTruthy();
    });
  });
