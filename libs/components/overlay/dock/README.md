...
| -------------------- | ----------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `items`              | `DynamoDockItem[]` (required) | —           | Each item: `{ label, icon?, disabled?, command?, badge? }`. `icon` falls back to the label's first letter when omitted. `badge` (string or number) renders a small corner badge and is folded into the tile's `aria-label` as `"<label> (<badge>)"`. |
