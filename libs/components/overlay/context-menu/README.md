...
<dg-context-menu (itemSelect)="onItemSelect($event)">
  ...
  <dg-menu-item value="copy" label="Copy" icon="⧉" />
...
| `itemSelect` | `DynamoMenuItemSelectEvent` (`{ value, label, disabled, icon? }`) | An enabled item is clicked or activated via Enter/Space — a plain snapshot, never the `DynamoMenuItem` component instance. |
