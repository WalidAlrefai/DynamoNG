...
// component — tag.html only ever binds `[class]="classes()"`.
//
// Same severity/variant/size shape as badge.styles.ts/chip.styles.ts —
// Badge and Chip each independently redeclare this identical cva() rather
// than sharing one token map, so Tag follows that same precedent instead of
...
// button.styles.ts/badge.styles.ts.
